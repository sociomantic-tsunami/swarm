    ...
    : ISwarmConnectionHandler
{
    ...
    ***************************************************************************/
    ...
    /***************************************************************************
        ...
        Handles the request according to handleRequest(), above, with the
        addition of stats tracking for the request.
        ...
        Params:
            request = request handler to run
            acquired = resources acquired while handling the request
            rq_name = name of request for stats tracking

    ***************************************************************************/

    deprecated("Call handleRequest() with the `stats` template argument set to "
        "RequestStatsTracking.Count and the `rq_name` argument set")
    protected void handleRequestTrackStats ( Resources, Acquired )
        ( IRequest request, Acquired acquired, cstring rq_name )
    {
        this.handleRequest!(Resources, Acquired, RequestStatsTracking.Count)(
            request, acquired, rq_name);
    }
